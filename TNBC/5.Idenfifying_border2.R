# identify_bordering_cells function
identify_bordering_cells2 <- function(spe_object, reference_cell, 
  ...
  # 폴리곤의 면적 계산
  polygon_areas <- calculate_polygon_area(ahull_polygon)
  
  # 계산된 면적을 spe_object의 속성으로 저장
  attr(spe_object, "area") <- polygon_areas
  print(polygon_areas)
  ...
  if (dim(data[which(data$Region=="Border"),])[1]!=0){
    if (isTRUE(plot_final_border)){
      plot(data[which(data$Region=="Border"), 
           ...
           pch = 19, cex = 0.5, main = paste(attr(spe_object, "name"),
                                             "bordering cells"))}
  }
  else{
  ...
  # 반환 값
  SPIAT::plot_cell_categories(spe_object, feature_colname = "Structure")
